...
### 3.2 설정 진입점: `src/config/stages/stage3/stage3.js`
...
```bash
node -e "const fs=require('fs');const b=fs.readFileSync('public/models/stage3/island8_compression.glb');const L=b.readUInt32LE(12);const g=JSON.parse(b.slice(20,20+L).toString('utf8'));(g.nodes||[]).forEach((n,i)=>{if(n.name)console.log(i,n.name);});"
```
