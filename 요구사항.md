...
> **정본 스펙**: [API.md](./API.md)
> **역할 분리**: 모니터는 **`POST .../start`**, **`GET .../current`**, **`POST .../complete`** 를 사용합니다. **`POST /api/request-monitor`** 는 **태블릿 전용**입니다.  
> **시작 화면 “도착” 토스트**만 예외로 **`GET /status`** 의 `monitors[monitorId].reservedWorry` 를 참고할 수 있으며, 이는 **gum_server가 필드·가용성을 보장**해 주어야 합니다(아래 “시작 화면 … 토스트” 절).
...
- 서버 스펙상 **예약만 있는 단계**는 `GET /current`가 `idle`만 줄 수 있으므로, 시작 화면에서 도착 토스트가 필요하면 **`GET /status`의 `reservedWorry`** 를 참고합니다.
- 구현: `src/pages/StartPage.jsx`에서 `/status`의 `reservedWorry`가 있으면 토스트 표시, 없으면 기존대로 `GET /current`의 `busy` + `worry`를 표시합니다.

### gum_server에 전달·합의할 것

- **프로덕션에서도** `GET /status` 응답에 **`monitors[monitor-1|monitor-2].reservedWorry`** 가 예약이 있을 때 채워지고, **`worryId`·`svgUrl`**(토스트·`/current`의 `worry`와 동일한 의미)가 일관되게 내려오는지 확인합니다.
- `/status`를 **운영에서 끄거나 축소**할 계획이면, 대신 **`GET /current`에 예약 단계를 노출**(예: `status: "reserved"` + `worry`)하거나 **전용 경량 API**를 두는 식으로 **스펙을 API.md에 먼저 올린 뒤** 프론트를 그에 맞게 바꿉니다.
...
### 로컬: MSW로 gum_server 모킹 (gum-frontend)

- 실행: `npm run dev:mock` (내부적으로 `VITE_ENABLE_MSW=true`)
- 시나리오: 주소에 `?mock=` 쿼리 — `idle` | `reserved` | `busy` | `flow`  
  - 예: `http://localhost:5173/start?mock=reserved` → `/status`에 `reservedWorry`만 있고 `/current`는 `idle` → 시작 화면에 **도착 토스트** 확인  
  - `flow`: 예약 → Stage3에서 `start` 호출 시 `busy`로 전환 → `complete` 후 대기
- 구현: `src/mocks/handlers.js`, `src/mocks/browser.js`, `public/mockServiceWorker.js`
- 기본 시나리오(쿼리 없음): `VITE_MSW_SCENARIO` 환경변수, 없으면 `reserved`
...
| 모니터 | `start`, `current`, `complete` (+ 시작 화면 토스트용 선택 `GET /status`) |

태블릿 쪽 상세는 별도 기획/문서를 따릅니다. (이 레포에 없을 수 있음)
...
| 동작 | 파일 |
| ---- | ---- |
| `postMonitorStart` / `fetchMonitorCurrent` / `postMonitorComplete` / `fetchGumServerStatus` | `src/lib/monitorCurrentApi.js` |
| Stage3 진입 시 `start` 후 폴링 | `src/stages/Stage3.js` (`setup`) |
| 시작 화면 폴링·토스트, `complete=1` 시 `complete` | `src/pages/StartPage.jsx` |
| Stage6 끝 → `/start?complete=1` | `src/pages/KioskPage.jsx`, `src/stages/Stage6.js` |

요구사항의 **`POST /start` 응답 JSON(`worry`)을 즉시 UI에 쓰기**는 선택 사항입니다. 현재 구현은 **폴링 `GET /current`**로 `busy` + `worry`를 받는 경로를 사용합니다(동작은 스펙과 호환).
